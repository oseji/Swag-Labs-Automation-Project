...
export class CheckoutPage {
    ...
    checkoutPageStepOneLocators = {
        ...
        closeErrorMessageButton: By.xpath(
            "//button[@data-test='error-button']",
        ),
    };
    ...
    private verifyValues(actual: number, expected: number, label: string) {
        expect(actual, `${label} should match calculated value`).to.equal(
            expected,
        );
    ...
    async checkoutStepTwo(productName: ProductName[]): Promise<void> {
